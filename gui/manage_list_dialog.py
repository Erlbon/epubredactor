"""
gui/manage_list_dialog.py

A small reusable Add/Remove dialog for the two Settings-menu management
screens (Add/Remove Genres, Add/Remove Languages). Both built-in
defaults and custom entries can be removed now -- removing a default
doesn't delete it, just hides it from the active list (a "(built-in)"
tag marks it as such), and "Restore Hidden Defaults" brings back
everything hidden this way in one step. Parametrized rather than
duplicated, since genres and languages are structurally the same
problem: a hideable default list plus a user-editable custom list --
entries are just (key, display_text) pairs, so it works for genres
(key == the genre name itself) and languages (key == the ISO code,
display_text includes the human name) alike.
"""
